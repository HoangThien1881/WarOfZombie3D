...
public class GasTankScript : MonoBehaviour
{
	...
	private IEnumerator Explode()
	{
		...
		Instantiate(destroyedGasTankPrefab, transform.position, transform.rotation);
		...
		foreach (Collider hit in colliders)
		{
			...
			if (hit.transform.CompareTag("GasTank"))
			{
			...
			// **If the explosion hits an object with the Health component**
			Health health = hit.transform.GetComponent<Health>();
			if (health != null)
			{
				health.TakeDamage(100); // Gây -100 máu cho zombie hoặc bất kỳ đối tượng nào có Health
			}
		}
